...
describe("POST /users", async function () {
  test("Creates a new user", async function () {
    ...
    const niceInDb = await User.findOne("nice");
    ["username", "email"].forEach((key) => {
      expect(dataObj[key]).toEqual(niceInDb[key]);
    });
  });
...
describe("PATCH /users/:username", async () => {
  test("Updates a single user's email with a selective update", async function () {
    const response = await request(app)
      .patch(`/users/${TEST_DATA.currentUsername}`)
      .send({ email: "newemail@gmail.com", _token: `${TEST_DATA.userToken}` });
  ...
  test("Prevents a bad user update", async function () {
    const response = await request(app)
      .patch(`/users/${TEST_DATA.currentUsername}`)
      .send({ notnot: false, _token: `${TEST_DATA.userToken}` });
    expect(response.statusCode).toBe(400);
  });
